...
ACE_TMAIN(int argc, ACE_TCHAR *argv[])
{
  try {
    ...
    DDS::Subscriber_var subscriber =
      participant->create_subscriber(subQos,
                                     0,
                                     OpenDDS::DCPS::DEFAULT_STATUS_MASK);
    ...
    Throughput::DataTypeDataReader_var reader_i =
      Throughput::DataTypeDataReader::_narrow(reader);
	...
	if(preTime.sec()>0) {
		...
		/*cout << "deltatime : " << deltaTime_sec
		     << "deltaReceived : " << deltaReceived 
		     << "Transfer rate : " << ((double)deltaReceived / 125000) / deltaTime_sec << endl;*/
	...
	//cout << time.sec()+(double)time.usec()/1000000 <<endl;
        //cout << preTime.sec()+(double)preTime.usec()/1000000 <<endl;
	preTime = time;
      }
